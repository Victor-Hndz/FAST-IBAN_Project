...
int main(int argc, char **argv) {
    ...
    for (time=0; time<NTIME; time++) { 
        ...
        #pragma omp parallel for num_threads(N_THREADS) private(iam, lat, lon, i, ini, final, bearing_count, bearing_count2, z_aux_selected) shared(np, z_in, lats, lons, size_x, size_y, time, resto, selected_points, filtered_points, step, scale_factor, offset) default(none) schedule(static)
