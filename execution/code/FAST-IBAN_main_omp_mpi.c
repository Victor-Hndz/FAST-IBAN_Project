...
int main(int argc, char **argv) {
    int ncid, retval, i, j, k, time, lat, lon, size_x, size_y, step, bearing_count, bearing_count2, id, rank, size;
    ...
    int time_start = (NTIME / size) * rank;
    int time_end = (NTIME / size) * (rank + 1);
    if (rank == size - 1) 
        time_end = NTIME; // El último proceso se encarga de cualquier resto.
    ...
    for (time=time_start; time<time_end; time++) { 
        ...
        #pragma omp parallel for num_threads(N_THREADS) private(lat, lon, i, bearing_count, bearing_count2, z_aux_selected) shared(z_in, lats, lons, size_x, size_y, time, selected_points, filtered_points, step, scale_factor, offset) default(none) schedule(static)
        ...
        int clusters_cont=0;
        for(i=0;i<id;i++) 
            if(clusters_aux[i].point_sup.point.lat >= 85.00 || clusters_aux[i].n_points == 1)
                clusters_cont++;
